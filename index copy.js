...
app.put("/api/showrooms/:id", (req, res) => {
  const showroomId = parseInt(req.params.id);
  const { name, location } = req.body;
  const showroomIndex = showrooms.findIndex(showroom => showroom.id === showroomId);
  if (showroomIndex !== -1) {
...
app.get("/api/vehicleFilters/:id", (req, res) => {
  ...
  const vehicleFilter = vehicleFilters.find(filter => filter.id === vehicleFilterId);
  if (vehicleFilter) {
...
app.post("/api/vehicleFilters", (req, res) => {
  ...
  const newVehicleFilter = { id: vehicleFilters.length + 1, vehicleId, filterId };
...
app.put("/api/vehicleFilters/:id", (req, res) => {
  const vehicleFilterId = parseInt(req.params.id);
  const { vehicleId, filterId } = req.body;
  const vehicleFilterIndex = vehicleFilters.findIndex(filter => filter.id === vehicleFilterId);
  if (vehicleFilterIndex !== -1) {
    vehicleFilters[vehicleFilterIndex] = { id: vehicleFilterId, vehicleId, filterId };
